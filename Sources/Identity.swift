...
@objc(AEPMobileEdgeIdentity) public class Identity: NSObject, Extension {
    ...
    public func readyForEvent(_ event: Event) -> Bool {
        if event.urlVariables, getSharedState(extensionName: IdentityConstants.SharedState.Configuration.SHARED_OWNER_NAME, event: event)?.value == nil { return false}
    ...
    func processGetUrlVariablesRequest(event: Event) {
        guard let configurationSharedState = getSharedState(extensionName: IdentityConstants.SharedState.Configuration.SHARED_OWNER_NAME, event: event)?.value else {
            Log.error(label: friendlyName, "\(#function) - Cannot process getUrlVariables request Identity event, configuration not found.")
            return
        }
        ...
        guard let orgId = configurationSharedState[IdentityConstants.ConfigurationKeys.EXPERIENCE_CLOUD_ORGID] as? String, !orgId.isEmpty else {
            Log.error(label: friendlyName, "\(#function) - Cannot process getUrlVariables request Identity event, experienceCloud.org is invalid or missing in configuration.")
            return
        }

        guard let ecid = state.identityProperties.ecid else {
            Log.error(label: friendlyName, "\(#function) - Cannot process getUrlVariables request Identity event, ECID is nil or not yet generated by the SDK.")
            return
        }
        let tsString = String(Int(Date().timeIntervalSince1970))
        let urlVariables = URLAppender.generateURLVariablesPayload(ts: tsString, ecid: ecid, orgId: orgId)
