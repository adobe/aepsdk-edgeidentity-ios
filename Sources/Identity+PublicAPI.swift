...
@objc public extension Identity {
    ...
    static func updateIdentities(with map: IdentityMap) {
        guard !map.isEmpty, let identityDict = map.asDictionary() else {
            Log.debug(label: IdentityConstants.FRIENDLY_NAME, "Identity - Unable to updateIdentites as IdentityMap is empty or could not be encoded to a dictionary.")
            return
        }
    ...
    static func removeIdentity(item: IdentityItem, withNamespace: String) {
        ...
        guard !identities.isEmpty, let identityDict = identities.asDictionary() else {
            Log.debug(label: IdentityConstants.FRIENDLY_NAME, "Identity - Unable to removeIdentity as IdentityItem is empty or could not be encoded to a dictionary.")
            return
        }
